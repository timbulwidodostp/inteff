...
program define logit_cc 
	...
	preserve;
		quietly {;
			tempvar  _ieltst logit_cc _ielse x12 phat xbeta phi linear 					   b1b4x2 b2b4x1 deriv11 deriv22 deriv44 d2F d3F derivc 				   deriv;
			...
			gen `deriv44'= `phi' + _b[`x1x2']*`d2F'*`x1'*`x2' + 							   `x2'*`b2b4x1'*`d2F' + `x1'*`b1b4x2'*`d2F' + 						   `b1b4x2'*`b2b4x1'*`x1'*`x2'*`d3F';
			...
			** Derivatives of other covariates ; 
			...
			local n 1;
			while `n'<=_N {;
				mkmat `deriv11' `deriv22' `deriv44' `101'-``nn'' 						`derivc' if _n==`n', matrix(`mat123'); 
			...
			rename `_ieltst' `logit_tst';
		      ...
		      rename `linear' `logit_linear';
	            ...
	            keep `logit_phat' `logit_linear'
                         `_lgt_cc' `logit_se' `logit_tst' 
      	 		;
           	** End "quietly" **;
       	};
		...
		** Save options **;
	      ...
	      if `"`savegraph1'"'!="no_savegraph1"{;
			twoway scatter `_lgt_cc' `logit_linear' `logit_phat', 
				sort msymbol(0 i) connect(. l) yline(0) 
	      ...
	      if `"`savegraph2'"'!="no_savegraph2"{;
			twoway scatter `logit_tst' `logit_phat', 
				sort ms(0 i) connect(. l) yline(-1.96 0 1.96) 
